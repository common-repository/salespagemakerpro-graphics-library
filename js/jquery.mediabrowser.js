(function($){
	...
	$.MediaBrowser = {
		...
		insertFile: function(){
			...
			var AT=$("form#fileform input#alttext").val();
			var IA=$("form#fileform input#imageal").val();
            ...
            if(editor == "tinymce"){
				...
				*/
				...
				ed_spmp .focus();
				var text='<img class="'+IA+'" src="'+URL +'" alt="'+AT+'" />';
				if (tinymce.isIE)
					ed_spmp.selection.moveToBookmark(tinymce.EditorManager.activeEditor.windowManager.bookmark);
				ed_spmp.execCommand('mceInsertRawHTML', false, text);
